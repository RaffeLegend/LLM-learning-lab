...
$$\text{scaled-loss} = \text{loss} \times S$$
...
$$\text{有效 batch} = \text{micro-batch} \times \text{accumulation-steps} \times \text{n-gpus}$$
...
$$M_{\text{KV}} = 2 \times n_{\text{layers}} \times n_{\text{kv-heads}} \times d_{\text{head}} \times \text{seq-len} \times \text{batch} \times \text{bytes}$$

实际例子——LLaMA-3 70B（$n_{\text{layers}}=80$，$n_{\text{kv-heads}}=8$，$d_{\text{head}}=128$，FP16）：
