...
$$\text{Total} = 2 \times n_{\text{layers}} \times \text{batch} \times \text{seq-len} \times d_{\text{model}} \times \text{bytes}$$
...
$$lr = d_{\text{model}}^{-0.5} \cdot \min\left(\text{step}^{-0.5},\; \text{step} \cdot \text{warmup-steps}^{-1.5}\right)$$
